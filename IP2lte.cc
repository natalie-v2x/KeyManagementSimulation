...
void IP2lte::triggerHandoverSource(MacNodeId ueId, MacNodeId targetEnb)
{
    ...
    LtePhyUe* ue = check_and_cast<LtePhyUe*>(getSimulation()->getModule(binder_->getOmnetId(ueId))->getSubmodule("lteNic")->getSubmodule("phy"));
    Transaction tx(UPDATE,nodeId_,targetEnb,ue->getPubKey());
