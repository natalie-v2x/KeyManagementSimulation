...
#include "stack/phy/packet/KeyGen.h"
#include "stack/bcManager/BlockchainManager.h"
...
void LtePhyEnb::initialize(int stage)
{
    ...
    else if (stage == 1)
    {
        ...
        revoker_= new cMessage("revoker");
        scheduleAt(NOW+REVOKE_CLOCK, revoker_);
        manager = veins::TraCIScenarioManagerAccess().get();
        //CNT_R = 18;
        //manager = getSimulation()->getModuleByPath("veinsManager");
    }
...
void LtePhyEnb::handleSelfMessage(cMessage *msg)
{
    ...
    else if (msg->isName("revoker"))
    {
        /*if (CNT_R == 2)
        {
            EV << "+++++++++++++++++++++++++++++++++++ debug ++++++++++++++++++++++++++++++++++++++\n";
        }*/
        std::map<std::string, cModule*> activeUe = manager->getManagedHosts();
        for (std::map<std::string, cModule*>::const_iterator i = activeUe.begin(); i != activeUe.end(); i++)
        {
            std::random_device rd; // https://www.cnblogs.com/chen-pi/p/17866160.html
            std::mt19937 gen(rd());
            std::bernoulli_distribution dist(0.5);
            //std::uniform_int_distribution<int> dist(0, activeUe.size()-1);
            bool result = dist(gen);
            if (result == true)
            {
                BlockchainManager* bcM = check_and_cast<BlockchainManager*>(getParentModule()->getSubmodule("bcManager"));
                unsigned short VID = static_cast<unsigned short>(std::strtoul(i->first.c_str(), NULL, 0));
                MacNodeId id = VID + UE_MIN_ID;
                if (getSimulation()->getModule((binder_->getOmnetId(id)))==nullptr)
                    continue;
                LtePhyBase* ue = check_and_cast<LtePhyBase*>(getSimulation()->getModule(binder_->getOmnetId(id))->getSubmodule("lteNic")->getSubmodule("phy"));
                Transaction tx(REVOKE, nodeId_, nodeId_, ue->getPubKey());
                //EV << "******************************** Revoking a vehicle and generating the transaction.\n";
                //Transaction tx(REVOKE, nodeId_, nodeId_, Eigen::VectorXd::Random(16));
                bcM->generateTransaction(tx);
                break;
            }
        }
        scheduleAt(NOW+REVOKE_CLOCK, msg);
        /*if (CNT_R > 0){
            scheduleAt(NOW+REVOKE_CLOCK, msg);
            EV <<"********************** " << CNT_R << " *******************************\n";
            CNT_R--;
...
bool LtePhyEnb::handleControlPkt(UserControlInfo* lteinfo, LteAirFrame* frame)
{
    ...
    if (lteinfo->getFrameType() == KEY_INIT)
    {
        handleKeyInit(lteinfo, frame);
        delete frame;
        return true;
    }
    return false;
...
void LtePhyEnb::handleKeyInit(UserControlInfo * lteinfo, LteAirFrame* frame){
    auto pktAux = check_and_cast<Packet*>(frame->decapsulate());
    auto header = pktAux->peekAtFront<KeyGen>();
    Transaction tx(header->getHash(), header->getTimestamp(), header->getSource(), header->getDestination(), header->getType(), header->getKey());
    BlockchainManager* bcM = check_and_cast<BlockchainManager*>(getParentModule()->getSubmodule("bcManager"));
    bcM->generateTransaction(tx);
