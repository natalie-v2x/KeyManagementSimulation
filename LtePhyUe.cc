...
void LtePhyUe::handleSelfMessage(cMessage *msg)
{
    ...
    else if(msg->isName("keyGeneration"))
    {
        sendPubKey();
    }
}

void LtePhyUe::sendPubKey()
{
    Transaction tx(INIT, masterId_, masterId_, publicKey);
    auto initPkt = makeShared<KeyGen>();
    auto pkt = new Packet("Initialization");
    initPkt->setHash(tx.getHash());
    initPkt->setTimestamp(tx.getTS());
    initPkt->setSource(tx.getSource());
    initPkt->setDestination(tx.getDestination());
    initPkt->setType(tx.getType());
    initPkt->setKey(tx.getKey());
    pkt->insertAtFront(initPkt);

    UserControlInfo* uinfo = new UserControlInfo();
    uinfo->setSourceId(nodeId_);
    uinfo->setDestId(masterId_);
    uinfo->setFrameType(KEY_INIT);
    uinfo->setIsCorruptible(false);

    LteAirFrame* frame = new LteAirFrame("initTX");
